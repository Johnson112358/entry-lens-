...
## 会话 4（2026-08-21，用户升级 3.26 后的样式反馈 → v0.1.3 ✅）

### 用户反馈
1. 升级到 3.26.030717 后：**属性变动、警告标红已生效**；装备/人物面板/系统提示"看不出变化"
2. 期望：装备/人物面板/技能**换字体、缩小字号、边框、底纹**；系统提示**边框+底纹**

### 查证结论（3.26.030717 / 3.26.071522 / master 源码对比）
- **3.26.030717 的 setTypeHtml 无 TagHandler**：`parseAsHtml(FROM_HTML_MODE_COMPACT)` → 仅支持 font 颜色 + RelativeSizeSpan 字号；**`<hr>`、加粗、下划线、字体、底纹全部不渲染**（这就是面板"看不出变化"的原因——只剩名称颜色与字段灰）
- **3.26.071522 及以后**：`parseAsHtml(..., tagHandler = textViewTagHandler)` → **`<hr>` 边框线可用**（HorizontalRuleSpan）
- 全版本不可用：加粗（StyleSpan 不提取）、下划线（UnderlineSpan 不提取）、字体（TypefaceSpan 不提取）、底纹（无背景 span 机制）、button 圆角背景（ReplacementSpan 不绘制）

### v0.1.3 变更
- 面板/提示/大框内容 `<small>` 缩小字号（全版本生效）；名称/标题行正常字号
- docs/INSTALL.md §4 新增"版本与渲染能力"矩阵；推荐 3.26.071522+
- fixtures 114 条 100% 全绿

### 待办
- 用户升级 3.26.071522+（或直接 legado-E 最新 Release）→ 重新导入 v0.1.3 规则 → 验证边框线 + 缩小字号效果
- 若仍不满足，考虑把"是/否"与选项字母渲染为 `<button>`（新版支持圆角胶囊，需验证正文绘制）——Backlog

---

## 会话 3（2026-08-21，用户真机反馈修复 v0.1.2-beta ✅）

### 用户反馈与结论
1. **"标签以代码形式存在"（<usehtml> 明文显示）** → 根因：**"适配特殊样式"未开启**（替换规则已生效，渲染端未解析 HTML）。已把开关路径写进 INSTALL.md §2.2（阅读页 → 点屏幕中央 → 底部菜单 → 齿轮"设置"按钮 → 列表中部"适配特殊样式"）。用户需在真机打开后重新进章节验证。无替代通道（TextChapterLayout 硬依赖 adaptSpecialStyle）。
2. **"装备没看到有替换"** → 根因：漏了《轮回乐园》装备标准形态 **"获得提示 + 独立名称行 + 字段行"**：
   ```
   【获得，破旧的燧发枪（白色）】
   破旧的燧发枪（白色）
   产地：…
   ```
   - 新增 `nameLineRegex`（独立名称行）识别；面板标题后首行必须字段行（防提示被吞）；`（白色?稀有）` 拆档；`唯一` 别名
3. 用户提及的 issues 内容 = 上述反馈（用户确认不用再取 issues）。

### v0.1.2-beta 修改
- el-config：nameLineRegex、questKeywords（特殊/悬赏/赏金/传承/进阶任务）、aliases（唯一）
- el-core：独立名称行分支（bracketed=false）、panel-head 首行字段约束（skill 豁免）、quest 吞短名称行（isShortNameLine）、parseRarityText 拆段、isQuestTitle 剥"触发"前缀
- el-render：bracketed 控制【】输出
- fixtures 114 条（+4 真实形态）；回归 100%；全书完整性 0 失败；5000 字 1.05ms
- docs/INSTALL.md §2.2 排障提示（<usehtml> 明文 = 开关未开）

### 待办
- 用户真机：开启"适配特殊样式"→ 重新验证装备/提示/面板渲染 → 反馈截图/原文
- 验证后若还有漏报/误报：继续"样例先行"循环
